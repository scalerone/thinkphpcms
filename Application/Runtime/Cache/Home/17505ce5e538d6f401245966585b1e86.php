		...
		<div class="position">
			<div class="container clear">
				<div class="left float_l">
					<p>
						<span class="h2">
							<?php $cates= M("category")->where("status=1")->order("sort ASC")->select();$cates=cateSort2Child($cates,78);foreach($cates as $key=>$cate_val): extract($cate_val);$index=$key+1;if($type==1) $url=U("/list/".$id);if($type==2) $url=U("/page/".$id); endforeach;?>
						</span>
		...
		<div class="content">
			<div class="container">
				<div class="new_title">
					...
					<span class="date">发布日期：<?php echo (date('Y-m-d H:i:s',$article["addtime"])); ?>    编辑：<?php echo ($article["author"]); ?></span>
				</div>
				<div class="con">
