						...
						<tr>
							...
							<td>
								<?php if(is_array($rules)): $i = 0; $__LIST__ = $rules;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="group">
										<h2><input type="checkbox" name="rules[]" value="<?php echo ($v["id"]); ?>"><?php echo ($v["title"]); ?></h2>
										<p class="sub">&nbsp;&nbsp;&nbsp;
											<?php if(is_array($v['child'])): $i = 0; $__LIST__ = $v['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($i % 2 );++$i;?><input type="checkbox" name="rules[]" value="<?php echo ($child["id"]); ?>" /><?php echo ($v["title"]); ?>&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
										</p>
